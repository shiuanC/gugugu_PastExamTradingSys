        ...
        <div id="body">
            ...
            <div>
                <form name="add_step1" method="POST">
                    ...
                    <!-- 填入科目 -->
                    <p><input style="margin-left:400px;" type="text" name="subject" placeholder = "課程名稱"/></p>
                    <!-- 填入科目代號 -->
                    <p><input style="margin-left:400px;" type="text" name="sub_id" placeholder = "課程代碼"/></p>
                    <!-- 填入老師 -->
                    <p><input style="margin-left:400px;" type="text" name="teacher" placeholder = "老師姓名"/></p>
                    <!--subjct type:0-必修 / 1-選修 / 2-通事--->  
                    <p style="margin-left:455px;" >
                        <select name="sub_type">
                            <option value="0">必修</option>
                            <option value="1">選修</option>
                            <option value="2">通識</option>
...
<?php
include("connect.php");
require('encoder.php');//取得加解密的檔案
//save the basic data of the article to database
if(isset($_POST['submit'])){
    ...
    if (!empty($title) && !empty($subject) && !empty($sub_id)&&!empty($sub_id) ){//確定要填的都有填
        ...
        if(!$result){
            ...
            $en = encrypt_url("p_time=$p_time&p_date=$p_date&sub_type=$sub_type",$key_url_md_5);//加密要用的get值
            //填完基本資料進入上傳圖片的檔案
