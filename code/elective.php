            ...
            <div class="header">
                ...
                <div>
                    <?php
						...
						#搜尋type為1（選修）的科目
						$sth2=$dbh->prepare('select subject,sub_id from article where sub_type= 1 ;');
						$sth2->execute();
						#當科目的count為0時，將type為1（選修）的科目印出，並將印出科目的count設為1
						while($result2=$sth2->fetch(PDO::FETCH_ASSOC)){
							$x=(string)$result2['sub_id'];
							$sub_id = $result2['sub_id'];
							$sub_id_en = encrypt_url("sub_id=$sub_id",$key_url_md_5);
							if($count[$x] == 0){
								echo "<h3 style='text-align:center;'><a href='art_list.php?".$sub_id_en."'>".$result2['subject'].'</a></h3>'.'<br>';
								$count[$x]=1;
							}
						}
