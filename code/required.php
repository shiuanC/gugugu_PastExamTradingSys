            ...
            <div class="header">
                ...
                <div>
                    <?php
						...
						#搜尋type為0（必修）的科目
						$sth=$dbh->prepare('select subject,sub_id from article where sub_type= 0 ;');
						$sth->execute();
						#當科目的count為0時，將type為0（必修）的科目印出，並將印出科目的count設為1
						#取得加密檔案
						require("encoder.php");
						while($result=$sth->fetch(PDO::FETCH_ASSOC)){
							$x=(string)$result['sub_id'];
							if($count[$x] == 0){
								$sub_id = $result['sub_id'];
								$sub_id_en = encrypt_url("sub_id=$sub_id",$key_url_md_5);
								if($count[$x] == 0){
									echo "<h3 style='text-align:center;'><a href='art_list.php?".$sub_id_en."'>".$result['subject'].'</a></h3>'.'<br>';
									$count[$x]=1;
								}
